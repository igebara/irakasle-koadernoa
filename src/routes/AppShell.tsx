  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="flex items-center gap-3 pl-3 border-l border-border hover:bg-secondary/50 rounded-lg px-2 py-1 transition-colors"
      >
        ...
        <ChevronDown className={`h-4 w-4 text-muted-foreground transition-transform hidden md:block ${open ? "rotate-180" : ""}`} />
      </button>

      {open && (
        <div className="absolute right-0 top-full mt-2 w-52 rounded-xl bg-card border border-border shadow-xl z-50 overflow-hidden" style={{ boxShadow: "0 8px 32px -8px rgba(0,0,0,0.2)" }}>
          <div className="px-4 py-3 border-b border-border bg-secondary/30">
            <p className="text-sm font-semibold">Sarah Reyes</p>
            <p className="text-xs text-muted-foreground mt-0.5">{t("topbar.role")}</p>
          </div>
          <div className="py-1">
            <button
              onClick={() => { setOpen(false); navigate({ to: "/settings" }); }}
              className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm hover:bg-secondary/60 transition-colors text-left"
            >
