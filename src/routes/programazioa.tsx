  ...
  const validate = (raw: string) => {
    ...
    try {
      ...
      const arr: unknown[] = Array.isArray(json)
        ? json
        : (json as Record<string, unknown>).units != null
          ? ((json as Record<string, unknown>).units as unknown[])
          : [json];
      if (!arr.length) throw new Error("Empty array");
      const units: Unit[] = arr.map((u: unknown, i: number) => {
        const row = u as Record<string, unknown>;
        if (!row.title) throw new Error(`Item ${i}: missing "title"`);
        return {
          id: typeof row.id === "string" ? row.id : `imported-${Date.now()}-${i}`,
          ...
          competencies: Array.isArray(row.competencies)
            ? (row.competencies as unknown[]).map((c: unknown) => {
                const comp = c as Record<string, unknown>;
                return {
                  compId: String(comp.compId),
                  indicatorIds: Array.isArray(comp.indicatorIds)
                    ? (comp.indicatorIds as unknown[]).map(String)
                    : [],
                };
              })
            : [],
